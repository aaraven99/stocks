...
- Modern Vercel-friendly static dashboard for top-ranked opportunities, active setups, watchlist, metrics, and safeguards.
...
> By default, snapshot generation is **real-data only** and will fail if Yahoo data cannot be fetched.
> Use `--allow-simulated-fallback` only for local testing.
...
- Operational safeguards (market-hours status, stale data alerts, fetch latency, failover mode).
...
### Automated refresh for “best right now” behavior

This repo includes a scheduled GitHub Action (`.github/workflows/refresh_snapshot.yml`) that rebuilds and commits `public/data/latest_snapshot.json` every 30 minutes on weekdays.

That gives Vercel a regularly refreshed snapshot without running a persistent Python server.
