...
def fetch_universe() -> List[str]:
    """
    Build scan universe using S&P 500 + Nasdaq-100 members.
    Falls back to a curated list if remote sources are unavailable.
    """
    ...
    headers = {"User-Agent": "Mozilla/5.0 (compatible; UniversalTradingWorkstation/1.0)"}
    ...
    try:
        sp_tables = _read_html_tables("https://en.wikipedia.org/wiki/List_of_S%26P_500_companies")
        sp500 = sp_tables[0]["Symbol"].astype(str).tolist()

        ndx_tables = _read_html_tables("https://en.wikipedia.org/wiki/Nasdaq-100")
        ndx = []
        for table in ndx_tables:
            cols = [str(c).strip().lower() for c in table.columns]
            if "ticker" in cols:
                ndx = table[table.columns[cols.index("ticker")]].astype(str).tolist()
                break
        if not ndx:
            raise ValueError("Could not locate Nasdaq-100 ticker table")

        cleaned = [s.replace(".", "-").strip().upper() for s in sp500 + ndx if s]
        merged = sorted(list(dict.fromkeys(cleaned)))
        if len(merged) < 300:
            raise ValueError("Universe size unexpectedly small")
        return merged
    except Exception:
        return fallback
