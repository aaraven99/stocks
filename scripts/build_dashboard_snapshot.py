...
def fetch_recent_intraday(ticker: str, interval: str = "5m", period: str = "59d") -> Tuple[pd.DataFrame, float, str | None]:
    return _download_with_retries(ticker, interval=interval, period=period)


def fetch_training_daily(ticker: str, years: int = 20) -> Tuple[pd.DataFrame, str | None]:
    end = pd.Timestamp.utcnow().tz_localize(None).to_pydatetime()
    start = (pd.Timestamp(end) - pd.DateOffset(years=years)).to_pydatetime()
    df, _, err = _download_with_retries(ticker, interval="1d", start=start, end=end)
    return df, err
...
def run_real_pipeline(
    ...
    if len(ready) < min_real_tickers:
        raise RuntimeError(
            f"Only {len(ready)} real-data tickers ready (required {min_real_tickers}). "
            f"Dropped: {dropped_tickers}"
        )
...
def build_snapshot(
    ...
    try:
        snapshot = run_real_pipeline(
            ...
            min_real_tickers=min_real_tickers,
        )
    except Exception as exc:  # noqa: BLE001
