def explain(features,model=None,n=10,feature_names=None,background=None):
 ...
 ranked=sorted(features.items(),key=lambda x:abs(float(x[1])),reverse=True)[:n]
 return [{'feature':k,'attribution':float(v),'method':'magnitude_fallback'} for k,v in ranked]
