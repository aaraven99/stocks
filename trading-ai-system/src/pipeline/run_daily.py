...
def main(test_mode=False,send_email=False):
 cfg=config();cfg['system']['test_mode']=bool(test_mode or cfg['system'].get('test_mode'));os.environ['TEST_MODE']='1' if cfg['system']['test_mode'] else '0';set_seed(cfg['system']['seed']);db=Database();log=get_logger();date=trading_date();run_id=f'daily_{date}_{uuid.uuid4().hex[:8]}';timings={};errors=[];sector_map=sectors();broker=AlpacaPaperBroker(db)
 ...
 def stage(name,fn,default=None):
  ...
  try:return fn()
  except Exception as exc:errors.append({'stage':name,'error':str(exc)});log.exception(name);return default
 ...
 for ticker in deep_tickers:
  df=frames[ticker];context={'macro':macro,'sentiment':sentiment_snapshot(ticker),'options':options_flow_proxy(ticker),'event':event_risk(ticker),'cross_asset':cross,'breadth':breadth};features=make_features(df,context);store.put(ticker,date,features);prediction=predict(features);mc=MonteCarloEngine(sims,cfg['system']['seed']+abs(hash(ticker))%10000).run(df.Close.pct_change().dropna(),regime['regime'],float(df.Close.iloc[-1]),reliability=reliability);stress=summarize(stress_run(ticker));checks=validate_probability(prediction);mc_check=validate_mc(mc['metrics'])
  db.upsert('montecarlo_metrics',{'ticker':ticker,'date':date,'engine_weights_json':mc['weights'],'metrics_json':mc['metrics'],'distribution_json':mc['distribution'][-5000:]},['ticker','date']);db.upsert('predictions',{'ticker':ticker,'date':date,'model_id':prediction['model_id'],'probability':prediction['probability'],'expected_return':prediction['expected_return'],'expected_drawdown':prediction['expected_drawdown'],'target_before_stop':prediction['target_before_stop'],'uncertainty':prediction['uncertainty'],'explanation_json':prediction['explanation']},['ticker','date','model_id']);db.upsert('probability_checks',{'date':date,'ticker':ticker,'passed':int(checks['passed'] and mc_check['passed']),'findings_json':checks['findings']+mc_check['findings']},['date','ticker'])
  ...
  if context['event']['event_risk']:db.upsert('event_risk_flags',{'ticker':ticker,'date':date,'flag':'earnings_proximity_proxy','severity':context['event']['event_risk'],'details_json':context['event']},['ticker','date','flag'])
  analogs=nearest_analogs(db,features,date);analog_prior=outcome_prior(analogs);store_analog(db,ticker,date,features);prediction['analog_prior']=analog_prior;results.append({'ticker':ticker,'price':float(df.Close.iloc[-1]),'sector':sector_map.get(ticker,'Unknown'),'returns':df.Close.pct_change().dropna().tail(60),'features':features,'quality':quality[ticker],'prediction':prediction,'mc':mc['metrics'],'stress_penalty':stress['penalty'],'analog_prior':analog_prior})
 ...
 report={'date':date,'regime':regime['regime'],'regime_forecast':regime,'decision':decision,'picks':alloc[:12],'debate':debate,'narrative':llm,'kill_switch':kill,'universe_health':{'requested':len(universe),'loaded':len(frames),'deep_analyzed':len(deep_tickers),'failure_rate':failure_rate,'source':source,'failover':failover},'analytics':analytics};ensure_artifacts();html=html_build(report);write_text('daily_report.html',html);write_text('daily_report.md',md_build(report));equity_chart(db.rows('SELECT * FROM equity_curve ORDER BY date')).write_html(ensure_artifacts()/'equity_curve_chart.html',include_plotlyjs='cdn');manifest=build_manifest(cfg,{'run_id':run_id,'universe_source':source,'loaded':len(frames),'deep_tickers':deep_tickers,'timings':timings});write_json('manifest.json',manifest);db.upsert('reproducibility_manifests',{'run_id':manifest['run_id'],'date':date,'manifest_json':manifest,'created_at':manifest['created_at']},['run_id'])
