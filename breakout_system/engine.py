...
class IndicatorEngine:
    ...
    def add_indicators(df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Higher-timeframe alignment proxy using slow resampled trend (rolling mean of rolling mean).
...
class ModelManager:
    ...
    def _trade_r_returns(proba: np.ndarray, y_true: np.ndarray, threshold: float = 0.75) -> np.ndarray:
        ...
        if not np.any(take):
            return np.array([0.0])
        # Simplified R return proxy for classification labels.
    ...
    def train_with_walk_forward(self, frame: pd.DataFrame, features: List[str]) -> float:
        ...
        sharpe_oos = np.mean(trade_rs) / (np.std(trade_rs) + 1e-9) * np.sqrt(252)
        ...
        for start in range(0, len(data) - window - step + 1, step):
            ...
            fold_sharpes.append(float(np.mean(r_fold) / (np.std(r_fold) + 1e-9) * np.sqrt(252)))
...
class Backtester:
    def run(
        ...
        return {
            ...
            "sharpe": float(np.mean(rs) / (np.std(rs) + 1e-9) * np.sqrt(252)),
        }
...
class RealTimeBreakoutEngine:
    ...
    def on_bar(
        ...
        # Strict entry: breakout close above resistance only.
        if entry <= resistance:
            return None
    ...
    def invalidate_broken_structures(self) -> None:
        ...
        for signal in self.active_signals:
            ...
            if df is None or df.empty:
                continue
            # Auto-invalidation if structure is broken.
            if float(df["close"].iloc[-1]) <= signal.stop:
                continue
            # Dynamic stop update (only tighter, never looser).
